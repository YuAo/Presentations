...
public extension CGPath {
    static func makePath(svgPath: String, offset: CGFloat) -> CGPath {
        let path = CGMutablePath()
        path.applyCommands(from: SVGPath(svgPath), offset: offset)
        return path.copy()!
    }
}
    ...
    func applyCommands(from svgPath: SVGPath, offset: CGFloat) {
        for command in svgPath.commands {
            ...
            case .line:
                addLine(to: point)
            case .quadCurve:
                addQuadCurve(to: point, control: controlPoint1)
            case .cubeCurve:
                addCurve(to: point, control1: controlPoint1, control2: controlPoint2)
            case .close:
                closeSubpath()
            }
        }
